...
// v21: toda el área de portada + artista + canción abre la vista ampliada.
const playerInfo=document.getElementById('player-info');
let playerInfoPointerHandled=false;

function activatePlayerInfo(event){
  // El botón de reproducción y otros controles siguen siendo independientes.
  if(event?.target?.closest?.('button,a,input,select,textarea')) return;
  openCoverModal(event);
}

// pointerdown hace que el tap responda inmediatamente en Android/iPhone.
playerInfo?.addEventListener('pointerdown',event=>{
  if(event.pointerType==='mouse' && event.button!==0) return;
  playerInfoPointerHandled=true;
  activatePlayerInfo(event);
  setTimeout(()=>{playerInfoPointerHandled=false},350);
});

// click queda como respaldo para teclado, lectores y navegadores antiguos.
playerInfo?.addEventListener('click',event=>{
  if(playerInfoPointerHandled) return;
  activatePlayerInfo(event);
});
