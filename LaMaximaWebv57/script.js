...
addEventListener('DOMContentLoaded',()=>{
  ...
  // Se conserva siempre el mismo elemento de audio. Los reintentos anteriores
  // reemplazaban el reproductor mientras el usuario empezaba a deslizar y el
  // navegador perdía la autorización del gesto.
  if(!radio?.src){
    radio.src=freshStreamUrl();
    radio.preload='auto';
    radio.load();
  ...
  // Intento normal de autoplay, sin recrear el audio si el navegador lo bloquea.
  radio.muted=false;
  radio.volume=1;
  try{
    const autoplayAttempt=radio.play();
    autoplayAttempt?.catch?.(()=>{
      streamStatus.textContent='DESLIZA O TOCA PARA INICIAR';
    });
  }catch(_){
    streamStatus.textContent='DESLIZA O TOCA PARA INICIAR';
  }
...
function radioIsAudible(){
  return Boolean(radio && !radio.paused && !radio.muted && radio.volume>0 && !userMutedPause);
}

function unlockAudio(event){
  if(userMutedPause || radioIsAudible()) return;
  ...
  // No se cambia la URL, no se llama load() y no se usa ningún retraso aquí.
  // play() ocurre directamente dentro del toque físico que inicia el scroll.
  radio.muted=false;
  radio.volume=1;
  ...
  try{
    ...
    if(attempt && typeof attempt.then==='function'){
      attempt.then(()=>{
        ...
        streamStatus.textContent='TOCA LA PANTALLA PARA INICIAR';
      });
    }
  }catch(_){
    streamStatus.textContent='TOCA LA PANTALLA PARA INICIAR';
...
function removeUnlockListeners(){
  unlockEvents.forEach(type=>document.removeEventListener(type,unlockAudio,true));
}
unlockEvents.forEach(type=>document.addEventListener(type,unlockAudio,{capture:true,passive:true}));
...
function stopRadioCompletelyForTv(){
  // Cierra realmente la conexión de Icecast antes de iniciar Máxima TV.
  // A diferencia de stopRadio(), aquí no se conserva ningún streaming activo.
  userMutedPause=true;
  clearTimeout(recoveryTimer);
...
function showTvPreloader(){
  let loader=document.getElementById('maxima-tv-preloader');
  if(!loader){
    loader=document.createElement('div');
    loader.id='maxima-tv-preloader';
    loader.className='maxima-tv-preloader';
    loader.setAttribute('aria-live','polite');
    loader.innerHTML='<div class="maxima-tv-preloader-box"><span class="maxima-tv-preloader-spinner"></span><strong>PREPARANDO MÁXIMA TV</strong><small>CONECTANDO CON LA SEÑAL...</small></div>';
...
function hideTvPreloader(loader){
  if(!loader) return;
...
async function openMaximaTv(event){
  event?.preventDefault();
  event?.stopPropagation();
  if(tvOpening || tvFloating?.classList.contains('show')) return;
  tvOpening=true;

  lastTvTrigger=event?.currentTarget || event?.target?.closest?.('[data-maxima-tv]') || document.activeElement;

  // Primero se cierra totalmente la radio. Durante un segundo se muestra una
  // precarga visual para permitir que el navegador libere esa conexión.
  stopRadioCompletelyForTv();
  const loader=showTvPreloader();
