...
addEventListener('DOMContentLoaded',()=>{
  ...
  userMutedPause=false;
  connectMetadata();
  // Deja la URL cargada antes de cualquier gesto para que el primer toque o
  // desplazamiento pueda ejecutar play() sin recrear el reproductor.
  if(!radio?.src) prepareStream(false);
  setTimeout(()=>startRadio(false),350);
},{once:true});
...
function unlockAudio(event){
  // Cada evento del gesto hace su propio intento directo. No se bloquean los
  // touchmove/touchend posteriores si el navegador rechazó el touchstart.
  ...
  // La señal debe estar preparada antes del gesto. play() se llama directamente
  // dentro del evento táctil, sin setTimeout, await ni reconexiones intermedias.
  if(!radio?.src){
    radio.src=freshStreamUrl();
    radio.preload='auto';
    radio.load();
  ...
  try{
    ...
    if(attempt && typeof attempt.then==='function'){
      attempt.then(()=>{
        ...
        // No se inicia una cadena asíncrona aquí. El próximo touchmove o
        // touchend vuelve a intentarlo dentro de una nueva interacción real.
      });
    }
  }catch(_){ }
}

// touchmove es esencial cuando el dedo comienza a arrastrar la página. Se
// mantienen también touchstart y touchend porque cada navegador concede la
// activación de audio en una fase distinta del gesto.
['touchstart','touchmove','touchend','pointerdown','pointerup','click','keydown','wheel'].forEach(type=>{
