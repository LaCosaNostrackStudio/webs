...
function togglePlay(event){
  ...
  userMutedPause ? startRadio(false) : stopRadio();
}
...
async function unlockAudio(event){
  if(userMutedPause || audioUnlocked || unlockInProgress || (radio && !radio.paused)) return;
...
function enterLaMaxima(){
  if(!entryGateButton || entryGateButton.disabled) return;
  entryGateButton.disabled=true;

  // La interfaz entra inmediatamente. La radio comienza a conectar en segundo
  // plano y ya no bloquea la animación ni la retirada de la pantalla inicial.
  ...
  requestAnimationFrame(()=>{
    startRadio(false).catch(error=>{
      console.warn('No se pudo iniciar la radio al entrar:',error);
    });
  });
}
...
// Máxima TV: detiene la radio y abre la señal de video en un reproductor flotante.
const TV_STREAM_URL='https://streaming.lamaximafm.com:2020/hls/lamaximatv/lamaximatv.m3u8';
const tvFloating=document.getElementById('tv-floating');
const tvVideo=document.getElementById('maxima-tv-stream');
...
async function openMaximaTv(event){
  ...
  if(tvStatus){tvStatus.textContent='CONECTANDO CON MÁXIMA TV...';tvStatus.classList.remove('is-playing')}
  destroyTvStream();
  if(!tvVideo) return;

  if(tvVideo.canPlayType('application/vnd.apple.mpegurl')){
    tvVideo.src=TV_STREAM_URL;
  }else if(window.Hls?.isSupported()){
    tvHls=new Hls({enableWorker:true,lowLatencyMode:true,backBufferLength:30});
    tvHls.loadSource(TV_STREAM_URL);
    tvHls.attachMedia(tvVideo);
    tvHls.on(Hls.Events.ERROR,(_,data)=>{
      if(data?.fatal && tvStatus) tvStatus.textContent='NO SE PUDO CARGAR LA SEÑAL DE TV';
    });
  }else{
    tvVideo.src=TV_STREAM_URL;
  }

  try{await tvVideo.play()}catch(_){if(tvStatus) tvStatus.textContent='PRESIONA PLAY PARA VER MÁXIMA TV'}
...
function closeMaximaTv(event){
  ...
  // Al cerrar Máxima TV, la conexión de radio que quedó activa y silenciada
  // vuelve a escucharse automáticamente, sin abrir una sesión nueva.
  startRadio(false).catch(error=>{
...
tvVideo?.addEventListener('playing',()=>{if(tvStatus) tvStatus.classList.add('is-playing')});
tvVideo?.addEventListener('waiting',()=>{if(tvStatus){tvStatus.textContent='CONECTANDO CON MÁXIMA TV...';tvStatus.classList.remove('is-playing')}});
tvVideo?.addEventListener('error',()=>{if(tvStatus){tvStatus.textContent='NO SE PUDO CARGAR LA SEÑAL DE TV';tvStatus.classList.remove('is-playing')}});
