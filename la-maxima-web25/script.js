...
let playerInfoLocked=false;
let lastPlayerInfoOpen=0;

function activatePlayerInfo(event){
  if(event?.target?.closest?.('button,a,input,select,textarea')) return;

  const now=Date.now();
  if(playerInfoLocked || now-lastPlayerInfoOpen<700) return;

  playerInfoLocked=true;
  lastPlayerInfoOpen=now;
  openCoverModal(event);

  // Libera el bloqueo después de completar el gesto y la animación.
  setTimeout(()=>{playerInfoLocked=false},750);
}

playerInfo?.addEventListener('pointerdown',event=>{
  if(event.pointerType==='mouse' && event.button!==0) return;

  // Evita que el mismo toque genere pointerdown + click.
  event.preventDefault();
  activatePlayerInfo(event);
},{passive:false});

playerInfo?.addEventListener('click',event=>{
  // Respaldo para teclado y navegadores sin Pointer Events.
...
if(typeof coverModal!=='undefined'){
 coverModal.addEventListener('pointerdown',e=>{
   if(justOpenedModal){e.stopPropagation();}
 },true);
}
