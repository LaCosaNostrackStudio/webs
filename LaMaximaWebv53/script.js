...
function unlockAudio(event){
  // En móviles, "scroll" no autoriza audio. El play debe ejecutarse de forma
  // inmediata dentro del gesto que inicia el desplazamiento (touch/pointer).
  if(userMutedPause || radioIsAudible() || unlockInProgress) return;
  ...
  // No recrear la etiqueta de audio si ya tiene una señal cargada: así play()
  // ocurre en la misma llamada del gesto y conserva la autorización del móvil.
  if(!radio?.src) prepareStream(false);
  radio.muted=false;
  radio.volume=1;

  const playAttempt=radio.play();
  if(playAttempt && typeof playAttempt.then==='function'){
    playAttempt
      ...
      .catch(()=>startRadio(false))
      .finally(()=>setTimeout(()=>{unlockInProgress=false},250));
  }else{
    setTimeout(()=>{unlockInProgress=false},250);
  }
}

// Se escucha tanto en window como en document para cubrir Chrome/Opera Android,
// Samsung Internet y Safari iOS. La protección unlockInProgress evita duplicados.
['touchstart','touchend','pointerdown','click','keydown'].forEach(type=>{
  const options={capture:true,passive:type!=='keydown'};
  window.addEventListener(type,unlockAudio,options);
  document.addEventListener(type,unlockAudio,options);
});

// Respaldo para navegadores que no entregan touchstart al comenzar el arrastre.
// touchmove puede no ser una activación válida por sí solo, pero permite reintentar
// mientras el gesto todavía está activo en algunos navegadores Android.
document.addEventListener('touchmove',unlockAudio,{capture:true,passive:true});
