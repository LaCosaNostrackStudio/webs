...
function normalizeProjectEntry(p){
  return {
    ...
    client: normalizeText(p.client || p.description || p.name, 'Cliente'),
    projectType: cleanText(p.projectType || p.project || p.type || p.detail, 'Proyecto'),
    amount: asNumber(p.amount || p.price),
    status: ['pendiente','realizado','pagado'].includes(p.status) ? p.status : 'pendiente',
    incomeEntryId: p.incomeEntryId || ''
  };
}
...
function readProjects(){
  return (readJSON(PROJECTS_KEY, []) || []).map(normalizeProjectEntry).filter(p => p.date && p.client && p.amount);
}
...
function saveProjects(projects){
  const clean = (projects || []).map(normalizeProjectEntry).filter(p => p.date && p.client && p.amount);
  saveJSON(PROJECTS_KEY, clean);
}
function fullBackupObject(){
  const ingresos = readIngresos();
  const egresos = readEgresos();
  const proyectos = readProjects();
  return {
    version: 6,
    createdAt: new Date().toISOString(),
    note: 'Backup completo unificado. Incluye ingresos, egresos y trabajos/proyectos en un solo archivo.',
...
function exportUnifiedBackup(){
  ...
  downloadText(`backup_completo_finanzas_${new Date().toISOString().slice(0,10)}.txt`, JSON.stringify(backup, null, 2));
  setStatus('Backup completo descargado. Incluye ingresos, egresos y trabajos/proyectos.');
}
function rowsToHtml(title, rows, kind){
  const headers = kind === 'trabajos' ? ['Fecha','Semana','Mes','Cliente','Tipo de proyecto','Precio','Estado','Ingreso asociado'] : (kind === 'ingresos' ? ['Fecha','Semana','Mes','Cliente','Proyecto','Monto','Origen'] : ['Fecha','Semana','Mes','Categoría','Detalle','Monto','Origen']);
  const body = rows.map(e => {
    if(kind === 'trabajos') return `<tr><td>${escapeHTML(e.date)}</td><td>${escapeHTML(weekLabel(e.date))}</td><td>${escapeHTML(monthNameFromDate(e.date))}</td><td>${escapeHTML(e.client)}</td><td>${escapeHTML(e.projectType || '')}</td><td>${e.amount}</td><td>${escapeHTML(e.status || '')}</td><td>${escapeHTML(e.incomeEntryId ? 'Sí' : 'No')}</td></tr>`;
    return `<tr><td>${escapeHTML(e.date)}</td><td>${escapeHTML(weekLabel(e.date))}</td><td>${escapeHTML(monthNameFromDate(e.date))}</td><td>${escapeHTML(e.client)}</td><td>${escapeHTML(e.project || '')}</td><td>${e.amount}</td><td>${escapeHTML(e.source || '')}</td></tr>`;
  }).join('');
...
function exportUnifiedExcel(){
  ...
  const egresos = readEgresos();
  const proyectos = readProjects();
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body>${rowsToHtml('Ingresos', ingresos, 'ingresos')}<br>${rowsToHtml('Egresos', egresos, 'egresos')}<br>${rowsToHtml('Trabajos y proyectos', proyectos, 'trabajos')}</body></html>`;
  downloadText(`backup_completo_finanzas_${new Date().toISOString().slice(0,10)}.xls`, html, 'application/vnd.ms-excel;charset=utf-8');
  setStatus('Excel completo descargado. Incluye ingresos, egresos y trabajos/proyectos.');
}
function normalizeImportedBackup(raw){
  let ingresos = [], egresos = [], proyectos = [];
  if(raw && raw.ingresos){
  ...
  if(raw && (raw.trabajos || raw.proyectos)){
    const projectSection = raw.trabajos || raw.proyectos;
    const projectRaw = Array.isArray(projectSection) ? projectSection : (projectSection.registros || projectSection.registrosAgregados || []);
    proyectos = projectRaw.map(normalizeProjectEntry);
  }
  ...
  return { ingresos: ingresos.filter(e => e.date && e.amount), egresos: egresos.filter(e => e.date && e.amount), proyectos: proyectos.filter(p => p.date && p.client && p.amount) };
}
function importUnifiedFile(event){
  ...
  reader.onload = () => {
    try {
      ...
      saveProjects(imported.proyectos);
      setStatus(`Backup cargado. Ingresos: ${imported.ingresos.length}. Egresos: ${imported.egresos.length}. Trabajos/proyectos: ${imported.proyectos.length}.`);
      render();
    } catch(err) {
...
async function confirmWithPassword(message){
  const sure = confirm(message + '\n\nEsta acción elimina ingresos, egresos y trabajos/proyectos. No se puede deshacer.');
...
async function clearAllData(){
  const ok = await confirmWithPassword('Vas a borrar todos los ingresos, egresos y trabajos/proyectos guardados.');
  if(!ok) return;
  localStorage.setItem(GLOBAL_PRE_IMPORT_KEY, JSON.stringify(fullBackupObject(), null, 2));
  saveIngresos([]);
  saveEgresos([]);
...
function render(){
  ...
  const totalProyectos = proyectos.reduce((s,p)=>s+p.amount,0);
  const neto = totalIngresos - totalEgresos;
  document.getElementById('netTotal').textContent = currency(neto);
  document.getElementById('totalCount').textContent = `${ingresos.length + egresos.length + proyectos.length} registros totales`;
  document.getElementById('summaryCards').innerHTML = [
    ['Ingresos', currency(totalIngresos), `${ingresos.length} registros`],
    ['Egresos', currency(totalEgresos), `${egresos.length} registros`],
    ['Trabajos/proyectos', currency(totalProyectos), `${proyectos.length} registros`],
  ...
  document.getElementById('backupTable').innerHTML = `<tr><td>Ingresos</td><td>${ingresos.length}</td><td>${currency(totalIngresos)}</td></tr><tr><td>Egresos</td><td>${egresos.length}</td><td>${currency(totalEgresos)}</td></tr><tr><td>Trabajos/proyectos</td><td>${proyectos.length}</td><td>${currency(totalProyectos)}</td></tr><tr><td>Balance neto</td><td>${ingresos.length + egresos.length + proyectos.length}</td><td>${currency(neto)}</td></tr>`;
}
