...
function normalizeText(t){ return cleanText(t, 'Ingreso'); }
  ...
  (historical || []).forEach(month => {
    (month.weeks || []).forEach(week => {
      ...
      items.forEach(item => {
        ...
        out.push({ id: uid(), date, client: parts.client, project: parts.project, amount: asNumber(item[1]), source: 'histórico' });
      });
    });
  });
...
function normalizeEntry(e){
  const rawClient = e.client || e.description || e.name || 'Ingreso';
...
function fullBackupObject(entries = readEntries()){
  return { version: 4, createdAt: new Date().toISOString(), note: 'Backup completo unificado. Incluye cliente y proyecto separados.', metaSemanal: META, registros: entries };
}
...
function statusBadge(total){
  if(total >= META) return '<span class="badge ok">En meta</span>';
  if(total >= META * .9) return '<span class="badge warn">Cerca</span>';
  return '<span class="badge low">Bajo meta</span>';
}
...
function editEntry(id){
  ...
  const client = prompt('Editar cliente:', entry.client); if(client === null) return;
  const project = prompt('Editar proyecto o trabajo:', entry.project || ''); if(project === null) return;
  ...
  if(!updated.date || !updated.client || !updated.amount){ alert('No se editó. Verifica fecha, cliente y monto.'); return; }
...
function renderDashboard(){
  ...
  document.getElementById('summaryCards').innerHTML = [
    ['Total anual', currency(totalYear), `Meta anual: ${currency(annualGoal)}`], ['Meta mensual', currency(monthlyGoal), 'Calculada con 4 semanas por mes'], ['Promedio semanal', currency(avgWeek), avgWeek >= META ? 'Sobre la meta' : 'Debajo de la meta'], ['Semanas en meta', above, `${above} de ${weeks.length} semanas`], ['Semanas bajo meta', below, `${below} de ${weeks.length} semanas`], ['Mejor semana', best ? `${best.date} ${best.month}` : 'Sin data', best ? currency(best.total) : 'RD$0'], ['Peor semana', worst ? `${worst.date} ${worst.month}` : 'Sin data', worst ? currency(worst.total) : 'RD$0'], ['Proyección anual', currency(projection), monthComp.text]
...
function renderClients(entries){
  ...
  document.getElementById('clientsTable').innerHTML = rows.length ? rows.map((c,i)=>`<tr><td>${i+1}</td><td>${escapeHTML(c.client)}</td><td>${c.count}</td><td>${currency(c.total)}</td></tr>`).join('') : '<tr><td colspan="4">No hay clientes registrados.</td></tr>';
}
...
function drawChart(weeks){
  ...
  ctx.setLineDash([9,7]); ctx.strokeStyle = '#f5a524'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(pad.l,y(META)); ctx.lineTo(W-pad.r,y(META)); ctx.stroke(); ctx.setLineDash([]); ctx.fillStyle = '#9a6500'; ctx.font = 'bold 14px Arial'; ctx.fillText('Meta RD$21,000', W-170, y(META)-10);
...
function exportExcel(){
  ...
  const rows = [['Fecha','Semana lunes','Mes','Cliente','Proyecto','Monto','Origen'], ...entries.map(e => [e.date, weekLabel(e.date), monthNameFromDate(e.date), e.client, e.project || '', e.amount, e.source || 'manual'])];
  ...
  const blob = new Blob([html], {type:'application/vnd.ms-excel;charset=utf-8'}); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `ingresos_excel_${new Date().toISOString().slice(0,10)}.xls`; a.click(); URL.revokeObjectURL(a.href);
}
