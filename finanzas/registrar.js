...
function exportBackup(){ downloadText(`backup_ingresos_completo_${new Date().toISOString().slice(0,10)}.txt`, JSON.stringify(fullBackupObject(), null, 2)); document.getElementById('saveStatus').textContent = 'Backup completo descargado'; }
function exportExcel(){
  ...
  downloadText(`ingresos_excel_${new Date().toISOString().slice(0,10)}.xls`, html, 'application/vnd.ms-excel;charset=utf-8'); document.getElementById('saveStatus').textContent = 'Excel exportado';
}
...
document.getElementById('incomeForm').addEventListener('submit', e => {
  ...
  saveEntries([...readEntries(), entry]); e.target.reset(); document.getElementById('date').valueAsDate = new Date(); document.getElementById('saveStatus').textContent = 'Ingreso guardado. El dashboard se actualiza cada 10 segundos.';
});
