...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		fetch("tiempo.txt")
		    .then(response => response.text())  // Leer el contenido del archivo
		    .then(data => {
		        this.tiempo.text = data; // Asignar el contenido al campo de texto
		    })
		    .catch(error => console.error("Error al cargar titulo.txt:", error));
		...
		this.agregar.addEventListener("click", () => {
			...
			let tiempo = Number(this.tiempo.text);
			let tiempoEntregaActual = Number(this.tiempoentrega.text);
			this.tiempoentrega.text = tiempo * unidades + tiempoEntregaActual;
		...
		this.agregar.addEventListener("click", () => {
		    ...
		    if (numeroPedido >= 1 && numeroPedido <= 11) {
				...
				localStorage.setItem('tiempo' + numeroPedido, this.tiempo.text);
				localStorage.setItem('tiempoentrega', this.tiempoentrega.text);
	...
	this.tiempoentrega.lineWidth = 90;
	this.tiempoentrega.parent = this;
	this.tiempoentrega.setTransform(890.3,-81);

	this.tiempo = new cjs.Text("", "20px 'Arial'");
	this.tiempo.name = "tiempo";
	this.tiempo.lineHeight = 24;
	this.tiempo.lineWidth = 90;
	this.tiempo.parent = this;
	this.tiempo.setTransform(890.3,-124.05);
...
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/agregador_atlas_1.png?1744200032902", id:"agregador_atlas_1"},
		{src:"https://code.jquery.com/jquery-3.4.1.min.js?1744200032953", id:"lib/jquery-3.4.1.min.js"},
		{src:"components/sdk/anwidget.js?1744200032953", id:"sdk/anwidget.js"},
		{src:"components/ui/src/textinput.js?1744200032953", id:"an.TextInput"}
	],
	preloads: []
};
